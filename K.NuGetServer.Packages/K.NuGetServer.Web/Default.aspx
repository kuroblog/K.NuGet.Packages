...
<body>
    ...
    <div>
        <fieldset style="width: 800px">
            <legend><strong>Generate nuget package file</strong></legend>
            Command:<br />
            <b>nuget pack ..\<span>{targetDirectory}</span>\<span>{targetProject}</span>.csproj -Build {-Symbols} -Properties Configuration=Release -OutputDirectory <span>{Packages}</span></b>
        </fieldset>
        <fieldset style="width: 800px">
            <legend><strong>Put nuget packages</strong></legend>
            Command:<br />
            <b>nuget push <span>{Packages}</span>\<span>{*.nupkg}</span> <span>{apikey}</span> <span>{http://target-nuget.site.cn/api/v2/package}</span></b>
        </fieldset>
    </div>
</body>
</html>
